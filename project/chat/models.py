...
class RoomManager(models.Manager):
    def create(self, object, title, desc):
        ...
        r = self.model(content_object=object, title=title, description=desc)
        r.save()
        return r
    ...
    def get_or_create(self, object, title, desc):
        '''see if object exists if not create a room if none exists'''
        try:
            return self.get_for_object(object)
        except Room.DoesNotExist:
            return self.create(object, title, desc)

class Room(UUIDIdMixin, TitleSlugDescriptionModel):
    content_type = models.ForeignKey(ContentType) #What kind of content is this related to
    object_id = models.UUIDField() # to which instace of the aforementioned object is this related
    ...
    def __unicode__(self):
        ...
        return "Room ID: %d | Room Title: %s | Room Desc: %s" % (self.pk, self.title, self.description)
...
class Message(UUIDIdMixin):
    '''A message that belongs to a chat room'''
    room = models.ForeignKey(Room, editable=False, related_name='messages')
